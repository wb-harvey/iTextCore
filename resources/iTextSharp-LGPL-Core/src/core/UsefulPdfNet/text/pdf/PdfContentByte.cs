...
using UsefulPdfNet.text.exceptions;
using UsefulPdfNet.text.pdf;
using UsefulPdfNet.text.pdf.intern;
    ...
    public class PdfContentByte {
        ...
        */
        ...
        public void Rectangle(Rectangle rectangle) {
            ...
            if (rectangle.UseVariableBorders) {
                ...
                if (rectangle.BorderWidth != UsefulPdfNet.text.Rectangle.UNDEFINED) {
                ...
                if (rectangle.HasBorder(UsefulPdfNet.text.Rectangle.BOX)) {
                    this.Rectangle(x1, y1, x2 - x1, y2 - y1);
                }
                    // if the border isn't a rectangle, the different sides are added apart
                else {
                    if (rectangle.HasBorder(UsefulPdfNet.text.Rectangle.RIGHT_BORDER)) {
                        MoveTo(x2, y1);
                        LineTo(x2, y2);
                    }
                    if (rectangle.HasBorder(UsefulPdfNet.text.Rectangle.LEFT_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x1, y2);
                    }
                    if (rectangle.HasBorder(UsefulPdfNet.text.Rectangle.BOTTOM_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x2, y1);
                    }
                    if (rectangle.HasBorder(UsefulPdfNet.text.Rectangle.TOP_BORDER)) {
                        MoveTo(x1, y2);
                        LineTo(x2, y2);
                    }
                }
        ...
        */
        ...
        public float GetEffectiveStringWidth(String text, bool kerned) {
            ...
            //global::System.out.Println("String width = " + Float.ToString(w));
            return w;
        }
        ...
        */    
        ...
        public void Transform(global::System.Drawing.Drawing2D.Matrix tx) {
            float[] c = tx.Elements;
